...
export default function ManagerDialog({ hackathonId, search, setSearch, filteredUsers, setAllUsers }: { hackathonId: string, search: string, setSearch: React.Dispatch<React.SetStateAction<string>>, filteredUsers: UserSearchResult[], setAllUsers: React.Dispatch<React.SetStateAction<UserSearchResult[]>> }) {
    const [selected, setSelected] = useState<userId[]>([])
    const [page, setPage] = useState(1)
    
    const nonManagers = filteredUsers.filter(u =>
    ...
    function applyChanges() {
        ...
        const makeManager = async (id: userId) => {
            ...
            if (!currentHackathonParticipant.judge) {
                const currentJudge = await createJudge(currentHackathonParticipant.id, JudgeRole.MANAGER)
                if (!currentJudge) return
                currentHackathonParticipant.judge = currentJudge
            } else if (currentHackathonParticipant.judge.role == JudgeRole.JUDGE) {
                const res = await setJudgeType(currentHackathonParticipant.judge.id, JudgeRole.MANAGER)
                if (!res) return
                currentHackathonParticipant.judge.role = JudgeRole.MANAGER
            }

            setAllUsers((prev) => prev.map(u => (u.id === currentUser.id ? currentUser : u)));
            setSelected((prev) => prev.filter(id => (id !== currentUser.id)))
            if (selected.length === 0) toast.message(`Successfully promoted ${selectedCount} users to managers`)
        }
        ...
        <Dialog onOpenChange={() => {
            ...
            <DialogContent>
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Select users to promote to managers for this hackathon.
                    </DialogDescription>
                </DialogHeader>
                <input
                    type="text"
