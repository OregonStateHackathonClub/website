...
export default function UserInnerSheet({
  user,
  hackathonId,
  setUser,
}: {
  ...
  hackathonId: string;
  setUser: React.Dispatch<React.SetStateAction<UserSearchResult | null>>;
}) {
  function getParticipant() {
    return user.hackathonParticipants.find(
  ...
  async function deleteHackathonParticipant() {
    ...
    try {
      ...
      if (res) {
        ...
        setUser((prev) => ({
          ...prev!,
          hackathonParticipants: prev!.hackathonParticipants.filter(
    ...
    user && (
      <SheetContent>
        ...
        <div className="flex justify-between gap-2.5">
          <Label
            htmlFor={`${user.id}-superadmin`}
            className="w-15 flex justify-end flex-1"
          >
            User
          </Label>
          <Switch
          ...
          />
          <Label htmlFor={`${user.id}-superadmin`} className="flex-1">
            Superadmin
          </Label>
        </div>
        {hackathonId && (
          <>
            {getParticipant()?.judge ? (
              <>
                <div className="flex justify-between gap-2.5">
                  ...
                  <Switch
                    ...
                    checked={
                      getParticipant()?.judge?.role === JudgeRole.MANAGER
                    }
                    onCheckedChange={async (checked) => {
                      const role = checked
                        ? JudgeRole.MANAGER
                  ...
                  />
                  <Label htmlFor={`${user.id}-judge-type`} className="flex-1">
                    Admin
                  </Label>
                </div>
              ...
              <div className="flex justify-center">
                <Button
                  variant={"outline"}
                  className="w-60"
                  onClick={() => addJudgeRole(JudgeRole.JUDGE)}
                >
                  Promote to Judge
                </Button>
              </div>
            ...
            <div className="flex justify-center">
              <Button
                variant={"destructive"}
                className="w-60"
                onClick={() => deleteHackathonParticipant()}
              >
